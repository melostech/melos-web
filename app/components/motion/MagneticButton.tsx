...
export function MagneticButton({
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.92 }}
      className={cn(
        "relative isolate inline-flex items-center justify-center overflow-visible rounded-xl px-5 py-3.5 text-sm font-bold transition focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-teal-300",
        primary
          ? "bg-teal-300 text-slate-950 shadow-[0_12px_40px_rgba(20,184,166,.24)]"
          : "border border-white/15 bg-white/[.06] text-white backdrop-blur-sm",
        className
      )}
    >
      <span className="relative z-10">{children}</span>

      <AnimatePresence key={burst}>
        {burst > 0 && (
          <>
            <motion.span
              aria-hidden="true"
              className={cn(
                ...
                primary ? "border-teal-100" : "border-violet-300"
              )}
              ...
              return (
                <motion.span
                  ...
                  className={cn(
                    ...
                    primary ? "bg-teal-100" : "bg-violet-300"
                  )}
