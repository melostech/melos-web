...
export function Reveal({ children, className, delay = 0, direction = "up" }: RevealProps) {
  ...
  return <motion.div className={className} initial={{ opacity: 0, ...(reduced ? {} : offset) }} whileInView={{ opacity: 1, x: 0, y: 0 }} viewport={viewportOnce} transition={{ ...revealTransition, delay }}>{children}</motion.div>;
}
